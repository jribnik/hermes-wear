 ...
 *
 * Sole consumer of HermesApiClient.observeMessages() — relays
 * payloads into the shared repository for UI, and posts system
 * notifications for background delivery.
 */
class HermesConnectionService : LifecycleService() {

    private lateinit var preferenceHelper: PreferenceHelper
    private val app = HermesWearApp.instance
    ...
    companion object {
        ...
        fun start(context: Context) {
            val intent = Intent(context, HermesConnectionService::class.java).apply { action = ACTION_START }
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) context.startForegroundService(intent)
            else context.startService(intent)
        }

        fun stop(context: Context) {
            val intent = Intent(context, HermesConnectionService::class.java).apply { action = ACTION_STOP }
    ...
    private fun startConnection() {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "HermesWear:ConnectionWakeLock").apply {
            acquire(10 * 60 * 1000L)
        }

        // REPOSITORY is the sole observeMessages() consumer — used by both UI and notifications
        repository.startObserving()

        // Listen for messages for notification posting (same SharedFlow, not a second channel consumer)
        observeJob?.cancel()
                ...
                when (payload.type) {
                    PayloadType.MESSAGE -> {
                        payload.message?.let { msg ->
                            if (msg.sender == Sender.HERMES) showMessageNotification(msg)
                        }
                    }
                    PayloadType.APPROVAL -> {
                        payload.approval?.let { approval -> showApprovalNotification(approval) }
                    }
                    PayloadType.HEARTBEAT -> {
                        updateNotification("Connected to Hermes")
                    }
                    PayloadType.STATUS -> { /* handled by ViewModel */ }
                }
            }
        }
    ...
    private fun createNotificationChannels() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val connectionChannel = NotificationChannel(CHANNEL_ID, CHANNEL_NAME, NotificationManager.IMPORTANCE_LOW)
                .apply { description = "Shows when Hermes is connected"; setShowBadge(false) }
            val messageChannel = NotificationChannel(MESSAGE_CHANNEL_ID, "Hermes Messages", NotificationManager.IMPORTANCE_HIGH)
                .apply { description = "New messages from Hermes"; enableVibration(true) }
            val approvalChannel = NotificationChannel(APPROVAL_CHANNEL_ID, "Hermes Approvals", NotificationManager.IMPORTANCE_HIGH)
    ...
    private fun buildServiceNotification(): Notification {
        val intent = Intent(this, com.hermes.wear.ui.MainActivity::class.java).apply { flags = Intent.FLAG_ACTIVITY_SINGLE_TOP }
        val pi = PendingIntent.getActivity(this, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Hermes Wear").setContentText("Connecting...")
    ...
    private fun updateNotification(text: String) {
        val notification = NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Hermes Wear").setContentText(text)
            .setSmallIcon(android.R.drawable.ic_dialog_info).setOngoing(true)
    ...
    private fun showMessageNotification(message: HermesMessage) {
        ...
        val intent = Intent(this, com.hermes.wear.ui.MainActivity::class.java).apply { flags = Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_NEW_TASK }
        val pi = PendingIntent.getActivity(this, message.id.hashCode(), intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        val notification = NotificationCompat.Builder(this, MESSAGE_CHANNEL_ID)
            .setContentTitle("Hermes").setContentText(message.text)
    ...
    private fun showApprovalNotification(approval: ApprovalRequest) {
        val intent = Intent(this, com.hermes.wear.ui.MainActivity::class.java).apply { flags = Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_NEW_TASK }
        val pi = PendingIntent.getActivity(this, approval.id.hashCode(), intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        val notification = NotificationCompat.Builder(this, APPROVAL_CHANNEL_ID)
            .setContentTitle("\u26A0\uFE0F Approval Required").setContentText(approval.command)
            ...
            .setPriority(NotificationCompat.PRIORITY_HIGH).setContentIntent(pi).setCategory(NotificationCompat.CATEGORY_ALARM)
